...
ordb = org.Mm.eg.db
ors ='mmu'
##log2 fold change (MLE): condition KD vs contral 
##Wald test p-value: condition KD vs contral 
run_dif = function(t1,t2){
  #####
  a =a[,-t1]
  dat = dat[,-t2]
  df = df[-t2,]
  ...
  run_enrich_plot = function(difgen,contrast){
    ...
    write.csv(ekk_up@result,paste0("ekk_up_",contrast,".csv"),quote = F)
    write.csv(ekk_down@result,paste0("ekk_down_",contrast,".csv"),quote = F)
    write.csv(ego_up@result,paste0("ego_up_",contrast,".csv"),quote = F)
    write.csv(ego_down@result,paste0("ego_down_",contrast,".csv"),quote = F)
  ...
  difgen = ext
  contrast = 'ext'
  run_enrich_plot(difgen,contrast)
